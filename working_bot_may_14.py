...
class EasyApplyBot:
    ...
    def pause_bot(self, reason=""):
        ...
        if not self.is_paused:
            ...
            # Create and display pause notification
            self.display_notification("⏸️ BOT PAUSED - Auto-resume in 30s", "yellow")
            
            # Add auto-resume timer
            self.resume_timer = threading.Timer(30.0, self.resume_bot, ["Auto-resume after timeout"])
    ...
    def display_notification(self, message, color="yellow", duration=None):
        ...
        # Escape single quotes first before formatting
        ...
        notification.textContent = '{escaped_message}';
    ...
    def start_apply(self, positions, locations, date_range="r2592000"):
        ...
        if config:
            ...
            self.personal_details = config.get('personal_details', {})
        self.fill_data()
                ...
                if is_present(question_locator):
                    ...
                    for question in questions:
                        try:
                            ...
                            label = question.find_element(By.XPATH, "./ancestor::div[contains(@class, 'fb-dash-form-element')]//label")
                            q_text = label.text.strip().lower()
                        except NoSuchElementException:
                            q_text = ""

                        # Get answer from config or personal details
                        answer = None
                        if 'first name' in q_text:
                            answer = self.personal_details['first_name']
                        elif 'last name' in q_text:
                            answer = self.personal_details['last_name']
                        elif 'phone' in q_text:
                            answer = self.personal_details['phone']
                        elif 'city' in q_text:
                            answer = self.personal_details['city']
                        else:
                            # Check config answers using substring matching
                            for config_q, config_a in self.answers.items():
                                if config_q.lower() in q_text:
                                    answer = config_a
                                    break
                        ...
                        if answer:
                            ...
                            # Default to smart random based on question type
                            if 'year' in q_text or 'experience' in q_text:
                                question.send_keys(str(random.randint(2, 5)))
                            elif 'scale' in q_text or 'rating' in q_text:
                                question.send_keys(str(random.randint(7, 10)))
                            else:
                                question.send_keys("-")  # Fallback
